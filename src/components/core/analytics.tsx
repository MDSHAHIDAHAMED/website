...
function PageViewTracker({ children }: Readonly<PageViewTrackerProps>): React.JSX.Element {
 ...
 */
export function Analytics({ children }: Readonly<AnalyticsProps>): React.JSX.Element {
  if (!config.gtm?.id) {
    return <React.Fragment>{children}</React.Fragment>;
  }
