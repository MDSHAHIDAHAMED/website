...
export function MainNav({ color = 'evident', items = [] }: Readonly<MainNavProps>): React.JSX.Element {
        ...
        >
          <Stack direction="row" spacing={2} sx={{ alignItems: 'center', flex: '1 1 auto' }}>
            ...
            <Box component={RouterLink} href={paths.home} sx={{ display: { xs: 'none', md: 'inline-block' } }}>
              <DynamicLogo height={32} width={122} emblem />
            </Box>
...
export function NotificationsButton(): React.JSX.Element {
  ...
  const unreadCount = useSelector((state) => state.notification?.unreadCount?.count);

  // Fetch unread count on mount (app startup)
  React.useEffect(() => {
...
function DropdownItem({
  ...
  pathname,
  title,
}: Readonly<DropdownItemProps>): React.JSX.Element {
  const active = isNavItemActive({ disabled, external, href, matcher, pathname });
  const isBranch = Boolean(items);
