      ...
      <CardContent>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3}>
          <Stack spacing={3} sx={{ flex: '0 0 auto', justifyContent: 'space-between', width: '240px' }}>
            <Stack spacing={2}>
              <AtomTypography color="success.main" variant="h2">
                +28%
              </AtomTypography>
              <AtomTypography color="text.secondary">
                increase in app usage with{' '}
                <AtomTypography color="text.primary" component="span">
                  6,521
                </AtomTypography>{' '}
            ...
            <div>
              <AtomTypography color="text.secondary" variant="body2">
                <AtomTypography color="primary.main" component="span" variant="subtitle2">
                  This year
                </AtomTypography>{' '}
    ...
    <Stack direction="row" spacing={2}>
      {bars.map((bar) => (
        <Stack direction="row" key={bar.name} spacing={1} sx={{ alignItems: 'center' }}>
          ...
          <AtomTypography color="text.secondary" variant="caption">
            {bar.name}
          </AtomTypography>
        </Stack>
      ))}
          ...
          (entry): React.JSX.Element => (
            <Stack direction="row" key={entry.name} spacing={3} sx={{ alignItems: 'center' }}>
              <Stack direction="row" spacing={1} sx={{ alignItems: 'center', flex: '1 1 auto' }}>
                ...
                <AtomTypography sx={{ whiteSpace: 'nowrap' }}>{entry.name}</AtomTypography>
              </Stack>
              <AtomTypography color="text.secondary" variant="body2">
